...
const commonCfdiErrors = [
  ...
  { signal: "RFC o total rechazado", title: "Datos de consulta con formato inválido", detail: "La consulta requiere RFC, total, UUID y sello en una forma específica.", action: "Confirma los atributos Emisor/Rfc, Receptor/Rfc y Comprobante/Total del XML." },
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#f5f1e8] text-[#171611]">
      ...
      <main className="relative z-10 mx-auto w-full max-w-[1340px] px-5 pb-20 sm:px-8 lg:px-12">
        ...
        <section id="como-funciona" className="screen-only grid gap-10 py-20 lg:grid-cols-[0.85fr_1.15fr] lg:py-28"><div><div className="eyebrow">Cómo funciona</div><h2 className="mt-5 max-w-md font-[var(--font-display)] text-5xl leading-[0.95] tracking-[-0.05em] sm:text-6xl">Una conexión breve con el <em className="font-normal text-[#b76421]">endpoint</em> público.</h2></div><div className="space-y-4"><InfoPanel number="1" title="El XML solo se lee aquí" text="El navegador extrae los cinco datos del comprobante; el archivo no se sube ni entra al historial." /><InfoPanel number="2" title="El servidor construye la expresión impresa" text="Agrupa re, rr, tt, id y fe, y envía POST HTTPS mediante la operación Consulta." /><InfoPanel number="3" title="El SAT devuelve un Acuse" text="La respuesta conserva CodigoEstatus, Estado, EsCancelable, EstatusCancelacion y ValidacionEFOS." /></div></section>
